-- 171_home_unified_feed_visa_stub_repeat.sql
-- Repeating visa-stub phrase (2+ occurrences) excluded from home unified feed.
  ...
  with src as (
    ...
    where p.moderation_status = 'safe'
      ...
      and not (
        ...
        or cardinality(regexp_split_to_array(
          lower(coalesce(p.title, '') || coalesce(p.excerpt, '') || coalesce(p.content, '')),
          '태국 비자 갱신은 정말 어렵다'
        )) >= 3
      )
    union all
    ...
    where j.moderation_status = 'safe'
      ...
      and not (
        ...
        or cardinality(regexp_split_to_array(
          lower(coalesce(j.title, '') || coalesce(j.excerpt, '')),
          '태국 비자 갱신은 정말 어렵다'
        )) >= 3
      )
    union all
    ...
    where m.moderation_status = 'safe'
      ...
      and not (
        ...
        or cardinality(regexp_split_to_array(
          lower(coalesce(m.title, '') || coalesce(m.excerpt, '')),
          '태국 비자 갱신은 정말 어렵다'
        )) >= 3
      )
  )
