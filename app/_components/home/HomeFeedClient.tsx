  ...
  return (
    <section className={styles.feed} id="home-community-feed" aria-labelledby="home-feed-h">
      <h2 className={styles.feedHead} id="home-feed-h">
        통합 라이브 피드{' '}
        <span style={{ fontWeight: 700, opacity: 0.85 }}>posts · jobs · market</span>
      </h2>

      {items.length === 0 && !err ? (
        <p className={styles.loadMore}>표시할 글이 없습니다.</p>
      ) : null}
            ...
            <Link key={`${p.kind}-${p.id}`} href={itemHref(p)} className={styles.feedRow}>
              ...
              <div>
                <div>
                  ...
                  <span style={{ fontSize: '0.58rem', color: 'var(--tj-muted)' }}>
      ...
      <div className={styles.loadMore} ref={sentinelRef} role="status">
        {batches >= MAX_BATCHES ? '더 불러오기를 마쳤습니다.' : loading ? '불러오는 중…' : ' '}
      </div>
    </section>
  );
}
